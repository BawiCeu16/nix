...
class ArtworkProvider extends ChangeNotifier {
  ...
  Uint8List? getCachedArtwork(int id, ArtworkType type, NixArtworkQuality quality) {
  ...
  Future<void> _fetchArtwork(int id, ArtworkType type, NixArtworkQuality quality) async {
    final key = _generateKey(id, type, quality);
    _pending.add(key);

    try {
      // Tiered logic moved to provider
      final format = quality == NixArtworkQuality.high ? ArtworkFormat.PNG : ArtworkFormat.JPEG;
